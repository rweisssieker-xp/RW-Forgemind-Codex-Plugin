...
Route market uncertainty, existing-app innovation, disruptive ideas, and MVP choice to `$forgemind-explore`. Route a selected outcome or architecture to `$forgemind-plan`. Route a concrete feature, bug, or fast MVP to `$forgemind-build`. Route a request to fully implement, finish, continue until done, or complete an outcome end to end to `$forgemind-complete`. Route review, testing, release, or evidence requests to `$forgemind-verify`. Route feedback and completed-work learning to `$forgemind-learn`.
