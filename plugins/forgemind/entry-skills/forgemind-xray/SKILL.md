...
Execute every safe detected local check and inspect the returned execution receipts before reporting a result. Xray is test-only: do not modify product source, configuration, credentials, or production systems. Isolated local or designated-test application data may be created or updated only as required by safe test flows; production and other non-test application data remain immutable. The score is informative only and does not block a release.
