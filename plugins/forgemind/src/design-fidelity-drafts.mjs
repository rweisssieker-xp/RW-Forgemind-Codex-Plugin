...
export async function prepareSelectedProductDesignProposal({ workspace, proposalSetId, proposalId, controlContractId }) {
  ...
  const implementation = { schemaVersion: 1, id: `implementation-${selection.id.slice('selection-'.length)}`, selectionId: selection.id, draftId: draft.id, controlContractId: controlContract.id, route: draft.route, viewport: draft.viewport, allowedExtensions: ALLOWED_UI_EXTENSIONS, requiredVerification: `design-fidelity run --draft-id ${draft.id} --control-contract ${controlContract.id} --control-observations '<observations-json>' --artifacts workspace --json`, correctionRule: 'Keep an edit only after safe project verification passes and the next measured difference decreases.' };
  ...
  return { schemaVersion: 1, status: 'implementation-ready', selection, draft, controlContract, implementation, nextAction: 'Implement only the selected draft in allowed UI files, then run the required measured verification.', errors: [] };
}
