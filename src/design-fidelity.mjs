...
export async function runDesignFidelity({ workspace, references, route, viewport, thresholdPercent, maxIterations, capture }) {
  ...
  const status = gaps.length ? 'blocked' : results.every(({ status }) => status === 'matched') ? 'matched' : 'needs-correction';
  const report = { schemaVersion: 1, status, contracts: results, corrections, gaps, generatedAt: new Date().toISOString(), errors: [] };
