...
export async function createMarketIntelligence({ workspace, projectProfile, financialModel }) {
  ...
  const result = {
    ...
    regulation: regulation(settings.region, category),
    ...
    claimBoundary: 'Market sizing, competitor gaps, pricing, buyer, regulatory, and sensitivity outputs are evidence-labelled scenarios, not market facts, legal advice, or forecasts.',
...
function regulation(region, category) { const eu = /dach|eu|europe/i.test(String(region ?? '')); return { value: eu ? 'Assess GDPR, data-processing, and applicable AI governance before processing personal or sensitive data.' : 'Regulatory requirements are unknown until region and data categories are supplied.', evidence: region ? 'inferred' : 'missing', sources: region ? ['configured region plus product category; not legal advice'] : ['no region configured'], category }; }
