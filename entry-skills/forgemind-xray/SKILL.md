...
For a running local web GUI, use the internal Browser to exercise visible user flows and retain screenshots or execution receipts as evidence. For a local native GUI or mobile emulator, use Computer Use. If the application, internal Browser, Computer Use, or the applicable control surface is unavailable, record the real Xray gap; do not claim a GUI result, pass, failure, or coverage without an execution receipt.
