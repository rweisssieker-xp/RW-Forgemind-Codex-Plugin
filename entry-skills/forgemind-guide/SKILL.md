...
Route market uncertainty, existing-app innovation, disruptive ideas, and MVP choice to `$forgemind-explore`. Route a selected outcome or architecture to `$forgemind-plan`. Route a concrete feature, bug, or fast MVP to `$forgemind-build`. Route review, testing, release, or evidence requests to `$forgemind-verify`. Route feedback and completed-work learning to `$forgemind-learn`.
