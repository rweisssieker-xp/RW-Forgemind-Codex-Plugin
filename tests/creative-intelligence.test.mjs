...
test('Evolve consolidates creative intelligence and existing-app MVP discovery', async () => {
  const explore = await readFile(path.join(root, 'playbooks', 'internal-journeys', 'forgemind-evolve', 'SKILL.md'), 'utf8');
...
test('Ship preserves the always-available bounded YOLO path', async () => {
  const build = await readFile(path.join(root, 'playbooks', 'internal-journeys', 'forgemind-ship', 'SKILL.md'), 'utf8');
...
test('Venture and Council preserve evidence and decision gates', async () => {
  const plan = await readFile(path.join(root, 'playbooks', 'internal-journeys', 'forgemind-venture', 'SKILL.md'), 'utf8');
  const verify = await readFile(path.join(root, 'playbooks', 'internal-journeys', 'forgemind-council', 'SKILL.md'), 'utf8');
